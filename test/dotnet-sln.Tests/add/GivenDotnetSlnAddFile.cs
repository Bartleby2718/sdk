...
{string.Format(CommandLineValidation.LocalizableStrings.UnrecognizedCommandOrArgument, "three.sln")}");
    ...
    public void WhenNestedFileIsAddedSolutionFoldersAreCreated(string solutionCommand)
    {
        ...
        cmd.Should().Pass()
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, fileToAdd, DefaultSolutionFolderName))
            .And.HaveStdErr("");
    ...
    public void WhenValidFileInSolutionRootIsPassedItGetsAdded(string solutionCommand, string testAsset)
    {
        ...
        cmd.Should().Pass()
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, "README", DefaultSolutionFolderName))
            .And.HaveStdErr("");
    }
...
#pragma warning restore CA2022 // Avoid inexact read
    ...
    public void WhenPassedMultipleFilesAndOneOfthemDoesNotExistItCancelsWholeOperation(string solutionCommand)
    {
        ...
        cmd.Should().Fail()
            .And.HaveStdErr(string.Format(CommonLocalizableStrings.CouldNotFindFile, "idonotexist.txt"))
            .And.HaveStdOut("");
    ...
    public void WhenSolutionFolderIsPassedFilesAreAddedThere(string solutionCommand)
    {
        ...
        cmd.Should().Pass()
            .And.HaveStdErr("")
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, fileToAdd, "TestFolder"));
    ...
    [Theory]
    [InlineData("sln", "TestFolder/InnerTestFolder/InnermostTestFolder")]
    [InlineData("solution", "TestFolder/InnerTestFolder/InnermostTestFolder")]
    [InlineData("sln", @"TestFolder\InnerTestFolder\InnermostTestFolder")]
    [InlineData("solution", @"TestFolder\InnerTestFolder\InnermostTestFolder")]
    public void WhenNestedSolutionFolderIsPassedButDoesNotExistTheSolutionFolderIsCreatedAndTheSolutionItemIsAdded(string solutionCommand, string solutionFolder)
    {
        var projectDirectory = _testAssetsManager
            .CopyTestAsset("TestAppWithSlnAndReadmeInSubDir", identifier: solutionCommand)
            .WithSource()
        ...
        cmd.Should().Pass()
            .And.HaveStdErr("")
            //.And.HaveStdOut(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, fileToAdd, "InnermostTestFolder"))
            ;
    ...
    [Theory]
    [InlineData("sln", "TestFolder/InnerTestFolder/InnermostTestFolder")]
    [InlineData("solution", "TestFolder/InnerTestFolder/InnermostTestFolder")] //TODO:
    [InlineData("sln", @"TestFolder\InnerTestFolder\InnermostTestFolder")]
    [InlineData("solution", @"TestFolder\InnerTestFolder\InnermostTestFolder")]
    public void WhenNestedSolutionFolderIsPassedAndDoesExistTheSolutionItemIsAdded(string solutionCommand, string solutionFolder)
    {
        var projectDirectory = _testAssetsManager
            .CopyTestAsset("TestAppWithSlnContainingNestedSolutionFolder", identifier: solutionCommand)
            .WithSource()
            .Path;
    ...
    public void WhenSolutionFolderIsPassedWithDirectorySeparatorFolderStructureIsCorrect(string solutionCommand, string solutionFolder, string testIdentifier)
    {
        ...
        cmd.Should().Pass()
            .And.HaveStdErr("")
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, fileToAdd, solutionFolder.Trim('/', '\\')));
    ...
    private void VerifySuggestionAndUsage(string solutionCommand, params string[] arguments)
    {
        ...
        const string fileArg = "README";
        var cmd = new DotnetCommand(Log)
            .WithWorkingDirectory(projectDirectory)
            .Execute([solutionCommand, "add", "file", ..arguments, fileArg, "App.sln"]);
        cmd.Should().Fail()
            .And.HaveStdOut("");
        cmd.StdErr.Should().BeVisuallyEquivalentTo(
            ...
            + (arguments.Length == 0
                ? $"  dotnet solution App.sln add file {fileArg}"
                : $"  dotnet solution App.sln add file {string.Join(" ", arguments)} {fileArg}")
        );
    }
}
