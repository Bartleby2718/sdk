...
public class GivenDotnetSlnAdd(ITestOutputHelper log) : SdkTest(log)
{
    private readonly Func<string, string> HelpText = (defaultVal) => $@"Description:
  Add one or more projects to a solution file.

Usage:
  dotnet solution [<SLN_FILE>] add [<PROJECT_PATH>...] [command] [options]
...
Commands:
  file <FILE_PATH>    Add one or more solution items to a solution file.
  folder <FOLDER_PATH>  Add one or more solution folders to a solution file.";
...
{string.Format(CommandLineValidation.LocalizableStrings.UnrecognizedCommandOrArgument, "three.sln")}");
    ...
    public void WhenNestedProjectIsAddedSolutionFoldersAreCreated(string solutionCommand)
    {
        ...
        cmd.Should().Pass()
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.ProjectAddedToTheSolution, projectToAdd))
            .And.HaveStdErr("");
    ...
    public void WhenDirectoryContainingProjectIsGivenProjectIsAdded(string solutionCommand, string testAsset)
    {
        ...
        var relativeProjectPath = Path.Combine("Lib", "Lib.csproj");

        var directoryPath = Path.GetDirectoryName(relativeProjectPath);
        var cmd = new DotnetCommand(Log)
            .WithWorkingDirectory(projectDirectory)
            .Execute(solutionCommand, "add", directoryPath);
        cmd.Should().Pass()
            .And.HaveStdOut(string.Format(CommonLocalizableStrings.ProjectAddedToTheSolution, relativeProjectPath))
            .And.HaveStdErr("");
...
#pragma warning restore CA2022 // Avoid inexact read
    ...
    private void VerifySuggestionAndUsage(string solutionCommand, params string[] arguments)
    {
        ...
        var cmd = new DotnetCommand(Log)
            .WithWorkingDirectory(projectDirectory)
            .Execute([solutionCommand, "add", ..arguments, "App.sln", projectArg]);
        cmd.Should().Fail();
        cmd.StdErr.Should().BeVisuallyEquivalentTo(
            ...
            + (arguments.Length == 0
                ? $"  dotnet solution App.sln add {projectArg}"
                : $"  dotnet solution App.sln add {string.Join(" ", arguments)} {projectArg}")
        );
        cmd.StdOut.Should().BeVisuallyEquivalentToIfNotLocalized("");
    }
}
